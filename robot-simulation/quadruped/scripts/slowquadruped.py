...
class Joint:
  ...
  def get_movement_increment(self, target_angle):
...
class QuadrupedCore:
  def __init__(self, hips, knees, ankles):
    ...
    self.leg_positions = [FRONT_LEFT, FRONT_RIGHT, BACK_LEFT, BACK_RIGHT]
    self.joint_positions = [HIP, KNEE, ANKLE]
    self.joints = []
    self.legs = []
  ...
  def slow_pose(self, position, hip_angle, knee_angle, ankle_angle, delay):
    ...
    hip = self.hips[position]
    knee = self.knees[position]
    ankle = self. ankles[position]

    dh = hip.get_movement_increment(hip_angle)
    dk = knee.get_movement_increment(knee_angle)
    da = ankle.get_movement_increment(ankle_angle)
    dt = delay / INCREMENTS
  ...
  def bend_up(self, position, delay=0):
    ...
    knee, ankle = knees[position], ankles[position]
    dk = knee.get_movement_increment(KNEE_BENTUP)
    da = ankle.get_movement_increment(ANKLE_BENTUP)
    dt = delay / INCREMENTS
  ...
  def move_joints_simultaneously(self, hip_angle, knee_angle, ankle_angle, delay):
    ...
    for leg in self.legs:
      dh = leg[HIP].get_movement_increment(hip_angle)
      dk = leg[KNEE].get_movement_increment(knee_angle)
      da = leg[ANKLE].get_movement_increment(ankle_angle)
      d_angles.append([dh, dk, da])

    for i in xrange(INCREMENTS):
      print 'move joints simultaneously: # ', i
      for position, leg in zip(self.leg_positions, self.legs):
