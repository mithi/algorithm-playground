...
def my_rotate!(array, amt)
  ...
  # size - |amt| if negative
  start = amt > 0 ? amt : amt + array.length
  last = start - 1 
  double_array = array + array[0..last] 

  array.each_index do |i|
    array[i] = double_array[start + i]
  end

  array
end

def my_rotate2!(array, amt)
  if amt > 0
      amt.times do
          el = array.shift
          array << el
